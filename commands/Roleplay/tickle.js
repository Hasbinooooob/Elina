...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    message.channel.send({embeds: [new MessageEmbed().setColor(ee.color).setImage(await anime.sfw.tickle()).setAuthor({name: `${message.author.username} tickles ${user.username}`}).setFooter({text: `Requested by ${message.author.tag}`, iconURL: message.author.displayAvatarURL()})]})
