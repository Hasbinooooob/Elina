...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    message.channel.send({embeds: [new MessageEmbed().setColor(ee.color).setImage(await anime.punch()).setAuthor({name: `${message.author.username} Punches ${user.username}`}).setFooter({text: `Requested by ${message.author.tag}`, iconURL: message.author.displayAvatarURL()})]})
    } catch (error) {
